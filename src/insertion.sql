-- Établir une connexion à la base de données Data Warehouse
CREATE EXTENSION IF NOT EXISTS postgres_fdw;

-- Créer un serveur étranger qui pointe vers votre Data Warehouse
CREATE SERVER warehouse_server
    FOREIGN DATA WRAPPER postgres_fdw
    OPTIONS (host 'atl-datamart-data-warehouse-1', port '5432', dbname 'nyc_warehouse');
    
-- Créer un mappage d'utilisateur pour la connexion
CREATE USER MAPPING FOR CURRENT_USER
    SERVER warehouse_server
    OPTIONS (user 'admin', password 'admin');

-- Créer un schéma pour les tables étrangères
CREATE SCHEMA IF NOT EXISTS warehouse;

-- Importer la table des courses de taxi du Data Warehouse
IMPORT FOREIGN SCHEMA public LIMIT TO (nyc_raw)
    FROM SERVER warehouse_server INTO warehouse;

-- Insertion de toutes les zones du fichier CSV
INSERT INTO dim_zone (zone_id, borough, zone_name, service_zone) VALUES
...
-- 1. Remplir la dimension temps
INSERT INTO dim_temps (date, heure, jour_semaine, mois, annee)
SELECT DISTINCT
...
FROM warehouse.nyc_raw;

-- 2. Créer les associations de location
...
FROM warehouse.nyc_raw
WHERE 
    pulocationid IN (SELECT zone_id FROM dim_zone) AND
    dolocationid IN (SELECT zone_id FROM dim_zone)
LIMIT 10000; -- Limitation à 10000 combinaisons pour éviter les problèmes de performances

-- 3. Charger la table de faits
...
SELECT
    t.temps_id,
    l.location_id,
    COALESCE(p.paiement_id, 5),  
    COALESCE(tr.tarif_id, 1),
...
FROM warehouse.nyc_raw r
JOIN dim_temps t ON 
    DATE(r.tpep_pickup_datetime) = t.date AND
    EXTRACT(HOUR FROM r.tpep_pickup_datetime) = t.heure
JOIN dim_location l ON 
    r.pulocationid = l.pu_location_id AND 
    r.dolocationid = l.do_location_id
LEFT JOIN dim_paiement p ON r.payment_type = p.payment_type
LEFT JOIN dim_tarif tr ON r.ratecodeid = tr.rate_code_id
LEFT JOIN dim_vendor v ON r.vendorid = v.vendor_code
WHERE 
    r.pulocationid IN (SELECT zone_id FROM dim_zone) AND
    r.dolocationid IN (SELECT zone_id FROM dim_zone)
LIMIT 100000; -- Limitation à 100000 courses pour éviter les problèmes de mémoire

-- Création d'index pour optimiser les performances des requêtes
CREATE INDEX IF NOT EXISTS idx_fact_temps ON fact_courses(temps_id);
CREATE INDEX IF NOT EXISTS idx_fact_location ON fact_courses(location_id);
CREATE INDEX IF NOT EXISTS idx_dim_zone_borough ON dim_zone(borough);
CREATE INDEX IF NOT EXISTS idx_dim_temps_date ON dim_temps(date);
CREATE INDEX IF NOT EXISTS idx_dim_temps_mois_annee ON dim_temps(mois, annee);